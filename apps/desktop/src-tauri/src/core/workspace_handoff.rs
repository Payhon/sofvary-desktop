...
use crate::core::workspace_handoff_prompt::{
    build_agents_md, build_claude_md, build_handoff_prompt, build_repair_prompt, build_task_md,
    build_tools_md,
};
...
pub enum WorkspaceHandoffError {
    ...
    PromptTemplate(#[from] crate::core::prompt_template::PromptTemplateError),
...
pub fn write_handoff_files(
    ...
    fs::write(sofvary_dir.join("task.md"), build_task_md(envelope)?)?;
...
fn resolve_packs_for_runtime(
    ...
    Ok((packs.runtime.manifest, packs.harness.manifest))
}
...
fn create_prompt_envelope(
    ...
    Ok(engine.create_envelope(requirement, manifest, runtime_pack, harness_pack)?)
}
...
mod tests {
    ...
    fn writes_required_handoff_files() {
        ...
        let prompt = build_handoff_prompt(&envelope, "Codex").expect("handoff prompt");
